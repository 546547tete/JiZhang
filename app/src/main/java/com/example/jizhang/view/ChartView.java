...
public class ChartView extends View{
    ...
    public int YPoint=Constant.point.y-200;     //原点的Y坐标
    public int XScale=55;     //X的刻度长度
    public int YScale=40;     //Y的刻度长度
    public int XLength=Constant.point.x-200;        //X轴的长度
    public int YLength=Constant.point.y-200;        //Y轴的长度
    ...
    protected void onDraw(Canvas canvas){
        ...
        for(int i=0;i*YScale<YLength ;i++)
        {
            ...
            try
            {
                canvas.drawText(YLabel[i]+"千" , XPoint-60, YPoint-i*YScale+5, paint);  //文字
            }
            catch(Exception e)
            {
            }
        ...
        drawText(canvas,"单位:万", XPoint-5, YPoint-YLength+YScale-5, paint,-90);
